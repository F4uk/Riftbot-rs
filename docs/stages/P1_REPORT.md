...
Status: implemented; awaiting GPT Gate 1
...
- `BookStore` accepts only increasing local versions and non-regressive receive timestamps.
...
| `cargo test --locked --all-targets --all-features` | 21 library tests passed; 0 failed; connectivity binary has 0 unit tests |
...
| `cargo run --locked --features p1-connectivity --bin p1-connectivity -- validate` | pass; all three feeds recovered healthy |

Hosted run `33167667553` repeated the policy, formatting, locked clippy, locked tests, and pinned
adapter compile checks successfully.
